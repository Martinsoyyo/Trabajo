...
#include "_DenseNet.h"
        ...
        struct _DenseLayerImpl : torch::nn::SequentialImpl {
            ...
            _DenseLayerImpl(int64_t num_input_features, int64_t growth_rate, int64_t bn_size, double drop_rate)
            {
                push_back("norm1", torch::nn::BatchNorm2d(num_input_features));
                push_back("relu1", torch::nn::Functional(torch::relu));
                push_back("conv1", torch::nn::Conv2d(Options(num_input_features, bn_size * growth_rate, 1).stride(1).bias(false)));
                push_back("norm2", torch::nn::BatchNorm2d(bn_size * growth_rate));
                push_back("relu2", torch::nn::Functional(torch::relu));
                push_back("conv2", torch::nn::Conv2d(Options(bn_size * growth_rate, growth_rate, 3).stride(1).padding(1).bias(false)));
        ...
        struct _DenseBlockImpl : torch::nn::SequentialImpl {
            _DenseBlockImpl(int64_t num_layers, int64_t num_input_features, int64_t bn_size, int64_t growth_rate, double drop_rate)
            {
        ...
        struct _TransitionImpl : torch::nn::SequentialImpl {
            _TransitionImpl(int64_t num_input_features, int64_t num_output_features) {
                ...
                push_back("relu ", torch::nn::Functional(torch::relu));
                push_back("conv", torch::nn::Conv2d(Options(num_input_features, num_output_features, 1).stride(1).bias(false)));
        ...
        DenseNetImpl::DenseNetImpl(int64_t num_classes, int64_t growth_rate, std::vector<int64_t> block_config, int64_t num_init_features, int64_t bn_size, double drop_rate)
        {
            // First convolution
            features = torch::nn::Sequential();
            features->push_back("conv0", torch::nn::Conv2d(Options(3, num_init_features, 7).stride(2).padding(3).bias(false)));
            features->push_back("norm0", torch::nn::BatchNorm2d(num_init_features));
            features->push_back("relu0", torch::nn::Functional(torch::relu));
            features->push_back("pool0", torch::nn::Functional(torch::max_pool2d, 3, 2, 1, 1, false));
            ...
            classifier->push_back(torch::nn::Linear(num_features, num_classes));
            classifier->push_back(torch::nn::LogSoftmax(1));
